...
export function initVeeTiles(
  ...
  // viewBox (0,0), the tile's clipped top-left corner (the "orb off the tile" bug).
  try { (function(){
    ...
    function ringPulse(color){
      ...
      if(ringgrp) ringAnim=ringgrp.animate([{opacity:0},{opacity:1,offset:.22},{opacity:.8,offset:.42},{opacity:0}],{duration:1000,easing:'ease-out',fill:'forwards'});
    ...
    function sendLight(){
      if(!feeds.length) return; // wireless variant: nothing to send a light down
  ...
  })(); } catch (e) { if (typeof console !== 'undefined') console.warn('[veeTiles] centre animation skipped:', e) }
  ...
  try { (function(){
    ...
    requestAnimationFrame(frame);
  })(); } catch (e) { if (typeof console !== 'undefined') console.warn('[veeTiles] orb layer skipped:', e) }
