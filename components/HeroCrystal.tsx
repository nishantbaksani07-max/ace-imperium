...
export default function HeroCrystal({
  ...
  useEffect(() => {
    ...
    /* Pause the loop while the tab is hidden or the canvas is scrolled fully
       offscreen — an ambient gem must never compete with the main thread/GPU
       for a tile tap. The clock keeps absolute time, so on resume the
       animation lands at the current phase (imperceptible for an ambient
       loop); dt is clamped in tick() so no jump integrates through. */
    let paused = false
    let offscreen = false
    const applyPause = () => {
      ...
      if (paused) {
        ...
        clock.getDelta() // swallow the paused gap so the next dt stays frame-sized
    ...
    function tick() {
      if (paused) return // applyPause() reschedules on resume
