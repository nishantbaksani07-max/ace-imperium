 ...
 */
export interface WhoopView {
  connected: boolean
  // The user has saved their own WHOOP app's client_id + client_secret, but
  // may not have completed the OAuth authorize step yet.
  hasCredentials: boolean
  // Vitality's callback URL — the user registers this in their own WHOOP app.
  redirectUri: string
  data: WhoopData | null
}
...
interface Props { view: WhoopView }

// Where to continue after keys are saved + WHOOP authorizes. Mirrors the
...
function friendlyError(code: string): string {
  switch (code) {
    case 'missing_credentials':
      return 'Add your WHOOP app keys first, then connect.'
    case 'invalid_state':
      return 'That connect attempt expired. Please try again.'
    case 'access_denied':
      return 'WHOOP access was declined. You can try connecting again.'
    default:
      return code
  }
}
...
export default function WhoopModule({ view }: Props) {
  ...
  useEffect(() => {
    ...
    else if (ok) setBanner({ tone: 'ok', text: 'Whoop connected' })
  }, [search])
  ...
  useEffect(() => {
    ...
    try {
      const d = view.data
      localStorage.setItem('vitality_whoop_v1', JSON.stringify({
        recovery: d.recovery,
        hrv: d.hrv,
  ...
  const onDisconnect = async () => {
    if (!confirm('Disconnect your Whoop account? Vitality will stop pulling new data.')) return
      ...
      <div className={dashboardStyles.shell}>
        <div className={fitnessStyles.header}>
          ...
          <div className={styles.titleRow}>
            <div>
              <h1 className={fitnessStyles.title}>Whoop</h1>
...
const WHOOP_DASHBOARD_URL = 'https://developer-dashboard.whoop.com/'

/**
 * BYO-credentials connect flow. WHOOP wouldn't grant our shared app
 * production access, so each user creates their own WHOOP developer app
 * (its owner can authorize their own WHOOP account in dev mode, no approval
 * needed) and pastes its keys here.
 *
 * Two states:
 *  · keys already saved → straight "Connect Whoop" CTA (+ a way to swap keys)
...
function ConnectPanel({
  ...
  const connectHref = `/api/whoop/connect?return=${encodeURIComponent(returnTo)}`

  if (hasCredentials && !showForm) {
    return (
      <section className={styles.connectPanel}>
        <p className={styles.connectIntro}>
          Your WHOOP app keys are saved. One tap to authorize — sign in with WHOOP,
          allow access, you&rsquo;re back here.
        </p>
        <p className={styles.connectSub}>We never see your WHOOP password. You can disconnect anytime.</p>
...
function CredentialsForm({ redirectUri, connectHref }: { redirectUri: string; connectHref: string }) {
  ...
  return (
    <section className={styles.connectPanel}>
      <p className={styles.connectIntro}>
        Connect WHOOP with <em>your own</em> developer keys. It takes about two minutes,
        and your <em>recovery</em>, <em>sleep</em>, and <em>strain</em> flow into Vitality from then on.
      </p>

      <ol className={styles.credSteps}>
        <li>
          Open the{' '}
          <a href={WHOOP_DASHBOARD_URL} target="_blank" rel="noopener noreferrer" className={styles.credLink}>
      ...
      <p className={styles.connectSub}>
        Your keys are stored server-side and used only to talk to WHOOP on your behalf.
        We never see your WHOOP password, and you can disconnect anytime.
      </p>
    </section>
  )
}
...
/**
 * WHOOP-required attribution. Their brand guidelines mandate that any data
 * sourced from WHOOP is clearly communicated as such. Monochrome wordmark,
 * undistorted — see app/app/fitness/whoop brand-compliance notes.
 */
function WhoopAttribution() {
  return (
    <span className={styles.whoopAttribution}>
      Data by <span className={styles.whoopMark}>WHOOP</span>
    </span>
  )
}
 ...
 */
function CoachCard({ d }: { d: WhoopData }) {
