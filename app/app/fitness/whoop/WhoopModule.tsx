...
export interface WhoopView {
  ...
  // Imperium's callback URL — the user registers this in their own WHOOP app.
  redirectUri: string
  data: WhoopData | null
}
...
export default function WhoopModule({ view }: Props) {
  ...
  const onDisconnect = async () => {
    if (!confirm('Disconnect your Whoop account? Imperium will stop pulling new data.')) return
  ...
  return (
    <section className={styles.connectPanel}>
      <p className={styles.connectIntro}>
        ...
        and your <em>recovery</em>, <em>sleep</em>, and <em>strain</em> flow into Imperium from then on.
      </p>
