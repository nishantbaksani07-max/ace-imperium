...
// A NEW worked example: the SLEEP-HOURS tile. It fills a gap the shipped examples
// leave open (the mentor example is a keyed-AI chat, the markets example is money).
// This is the duration/measure archetype done Fuel-grade: a serif hours-slept hero
// number, a status pill that reads honestly (idle before any night, good on a
// healthy night, caution when short), a real 7-night bar chart section, honest
// persistence through the host bridge, and exactly one duration stream reported.
//
...
.bignum{font-family:var(--font-serif);font-weight:400;font-size:clamp(3.5rem,13vw,5.2rem);line-height:.82;letter-spacing:-.02em;font-variant-numeric:tabular-nums}
...
.step{flex:none;width:48px;height:48px;border-radius:14px;border:1px solid var(--border-strong);background:var(--card);
  color:var(--fg);font-size:1.4rem;display:grid;place-items:center;
  transition:border-color var(--dur) var(--ease-premium),transform var(--dfast) var(--ease-premium)}
.step:hover{border-color:var(--mint);color:var(--mint)}
.step:active{transform:scale(.94)}
.stepRead{flex:1;text-align:center;font-family:var(--font-mono);font-size:.72rem;letter-spacing:.16em;text-transform:uppercase;color:var(--muted-strong)}
.btn{display:inline-flex;align-items:center;justify-content:center;gap:.5rem;width:100%;padding:.6875rem 1rem;
  border-radius:var(--radius-lg);font-size:.875rem;font-weight:600;line-height:1;background:var(--mint);color:var(--mint-ink);
  transition:background var(--dur) var(--ease-premium),transform var(--dfast) var(--ease-premium)}
...
.bfill.short{background:linear-gradient(180deg,rgba(245,158,11,.55),rgba(124,45,18,.35))}
...
@media (prefers-reduced-motion:reduce){*{animation:none!important;transition:none!important}
  .rise{opacity:1;transform:none}.bfill{transform:none}}
</style>
</head>
<body>
  <main class="shell">
    <div class="body">
      ...
      <div class="head rise" style="--i:0">
        <div class="eyebrow"><span class="num">01</span><span class="label">SLEEP</span></div>
      ...
      <section class="hero rise" style="--i:1">
        <h1 class="title">Sleep hours</h1>
        <div class="heroRow">
          <span class="bignum" id="value">0</span>
          <span class="pill idle" id="status"><span class="dot"></span>nothing yet</span>
        </div>
        <span class="unit">hours last night</span>
      </section>

      <div class="settings rise" id="settings" style="--i:2" hidden>
        <h3>nightly target</h3>
        <p>Set the hours you are aiming for. A short night reads amber, never red, and never a scold.</p>
        <label for="goal">target hours</label>
        <input id="goal" class="selectable" type="number" inputmode="decimal" step="0.5" min="4" max="12" placeholder="8">
      </div>

      <div class="steps rise" style="--i:2">
        <button class="step" id="minus" type="button" aria-label="half hour less">&minus;</button>
        <span class="stepRead" id="pending">8.0 h to log</span>
        <button class="step" id="plus" type="button" aria-label="half hour more">+</button>
      </div>
      <button class="btn rise" id="primary" style="--i:2" type="button">Log last night</button>
  ...
  (function(){
    ...
    function statusPill(v){
      ...
      if(v>=GOAL){pill('good',round1(v)+' h, rested');return}
      if(v<GOAL-1.5){pill('caution','short night');return}
      pill('good',round1(v)+' h logged');
    }
    ...
    function drawBars(){
      ...
      if(!logged.length){
        document.getElementById('section').innerHTML='<div class="empty"><div class="lead">nothing yet</div><div>log last night and your week starts here.</div></div>';
        return;
      }
      var max=1;wk.forEach(function(r){if(r.value>max)max=r.value});
      var cells=wk.map(function(r,idx){
        var ratio=r.value>0?Math.max(.05,r.value/max):0;var now=idx===6&&r.value>0;
        var short=r.value>0&&r.value<GOAL-1.5;
        var cls='bfill'+(now?' now':'')+(short&&!now?' short':'');
      ...
      var sum=0;logged.forEach(function(r){sum+=r.value});var avg=round1(sum/logged.length);
      var best=0;logged.forEach(function(r){if(r.value>best)best=r.value});
      var stat='avg '+avg+' h over '+logged.length+' of 7 nights, best '+round1(best)+' h';
      document.getElementById('section').innerHTML='<div class="bars">'+cells+'</div><div class="stat">'+stat+'</div>';
    }

    function render(){
      var v=get(today());
      document.getElementById('value').textContent=v>0?round1(v):'0';
      statusPill(v);
      document.getElementById('pending').textContent=round1(pending)+' h to log';
      drawBars();
      report(v);
    }
    ...
    function commit(){
      set(today(),round1(pending));writeStore();
      var m=document.getElementById('msg');m.className='msg';m.textContent='saved '+round1(pending)+' h for last night.';
      render();
    }
    ...
    document.getElementById('plus').addEventListener('click',function(){pending=Math.min(14,round1(pending+0.5));document.getElementById('pending').textContent=round1(pending)+' h to log'});
    document.getElementById('minus').addEventListener('click',function(){pending=Math.max(0,round1(pending-0.5));document.getElementById('pending').textContent=round1(pending)+' h to log'});
    document.getElementById('primary').addEventListener('click',function(){if(loaded)commit()});
    ...
    document.getElementById('goal').addEventListener('change',function(){
      var n=parseFloat(this.value);if(!isFinite(n)){return}
      n=Math.max(4,Math.min(12,n));mem.goal=n;GOAL=n;writeStore();render();
    });
...
test('sleep example: the tile is Vitality-grade (0 errors, 0 warnings)', () => {
  ...
  assert.equal(result.errors, 0, 'the sleep worked example must have zero floor errors');
  assert.equal(result.warnings, 0, 'the sleep worked example must have zero polish warnings');
});

test('sleep example: it clears the richness gate as Fuel-grade rich', () => {
  const r = richnessOf(HTML);
  if (!r.ok) console.log('\n  missing richness marks: ' + r.missing.join(', ') + '\n');
  assert.equal(r.ok, true, 'the sleep worked example must be Fuel-grade rich');
...
test('sleep example: it reports exactly one valid duration stream', () => {
  ...
  const v = validateReport({ key: 'sleep', label: 'Sleep hours', value: 7.5, date: '2026-07-02', kind: 'duration', goalDirection: 'up' });
...
import { mountReady as mountSleep } from './behaviorHarness.js';

test('sleep example behaves: renders and the stepper moves the hours to log', async () => {
  const now = new Date('2026-07-03T10:00:00');
  const h = await mountSleep(HTML, { now });
  assert.equal(h.errors.length, 0, `the sleep reference threw at mount: ${h.errors.join(' | ')}`);
  assert.ok(h.section().length > 0, 'the week section rendered');
  // +0.5h from the 8.0 default, then log -> the hero shows 8.5
  await h.click('#plus');
  await h.click('#primary');
  assert.equal(h.value(), 8.5, 'logging 8.5h shows 8.5 in the hero');
  assert.equal(h.errors.length, 0, 'no throw across the interaction');
  h.close();
});

test('sleep example behaves: reports through the host bridge and survives reopen', async () => {
  const now = new Date('2026-07-03T10:00:00');
  const h = await mountSleep(HTML, { now });
  await h.click('#plus');
  await h.click('#primary');
  ...
  assert.equal(h2.value(), 8.5, 'reopening keeps the logged hours (host persistence)');
  h.close();
  h2.close();
});
