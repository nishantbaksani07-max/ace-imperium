...
// A NEW worked example: the DEEP-WORK-SESSIONS tile. It fills a gap the shipped
// examples leave open (mentor is a keyed-AI chat, markets is money). This is the
// count/tally archetype done Fuel-grade: a serif today-count hero number, a status
// pill that reads honestly (idle before any focus block, good once the daily target
// is met), a real 7-day bar chart section, a soft-confirm undo so a mistap is never
// stuck, honest persistence through the host bridge, and exactly one count stream.
//
...
<title>Deep work sessions</title>
...
<body>
  <main class="shell">
    <div class="body">
      ...
      <div class="head rise" style="--i:0">
        <div class="eyebrow"><span class="num">01</span><span class="label">DEEP WORK</span></div>
      ...
      <section class="hero rise" style="--i:1">
        ...
        <span class="unit">sessions today</span>
      </section>

      <div class="settings rise" id="settings" style="--i:2" hidden>
        <h3>daily target</h3>
        <p>How many focus blocks are a good day. Hit it and the pill turns mint. Fall short and it stays a calm idle, never a scold.</p>
        <label for="goal">target sessions</label>
        <input id="goal" class="selectable" type="number" inputmode="numeric" step="1" min="1" max="12" placeholder="3">
      </div>

      <div class="actions rise" style="--i:2">
        <button class="undo" id="undo" type="button" aria-label="remove last session" disabled>&minus;</button>
        <button class="btn" id="primary" type="button">Log a session</button>
      </div>
  ...
  (function(){
    ...
    function statusPill(v){
      ...
      if(v>=GOAL){pill('good',v+' done, target met');return}
      pill('good',v+' in, keep going');
    }
    ...
    function drawBars(){
      ...
      if(!logged.length){
        document.getElementById('section').innerHTML='<div class="empty"><div class="lead">nothing yet</div><div>log your first focus block and the week starts here.</div></div>';
        return;
      }
      ...
      var cells=wk.map(function(r,idx){
        ...
        return '<div class="col"><div class="bwrap"><div class="bfill'+(now?' now':'')+'" style="transform:scaleY('+ratio.toFixed(3)+')"></div></div><span class="dlab">'+DOW[r.dow]+'</span></div>';
      }).join('');
      var sum=0;wk.forEach(function(r){sum+=r.value});var days=logged.length;
      var best=0;wk.forEach(function(r){if(r.value>best)best=r.value});
      var stat='this week '+sum+' sessions, '+days+' of 7 days, best day '+best;
    ...
    function add(delta){
      if(!loaded)return;
      var v=get(today())+delta;if(v<0)v=0;
      mem.days[today()]=v;writeStore();
      var m=document.getElementById('msg');m.className='msg';
      m.textContent=delta>0?'nice, one more block in the bank.':'removed one, no problem.';
      if(delta>0)popNum();
      render();
    }
    ...
    document.getElementById('primary').addEventListener('click',function(){add(1)});
    document.getElementById('undo').addEventListener('click',function(){if(get(today())>0)add(-1)});
    ...
    document.getElementById('goal').addEventListener('change',function(){
      ...
      n=Math.max(1,Math.min(12,n));mem.goal=n;GOAL=n;writeStore();render();
    });
...
test('deep-work example: the tile is Vitality-grade (0 errors, 0 warnings)', () => {
  ...
  assert.equal(result.errors, 0, 'the deep-work worked example must have zero floor errors');
  assert.equal(result.warnings, 0, 'the deep-work worked example must have zero polish warnings');
});

test('deep-work example: it clears the richness gate as Fuel-grade rich', () => {
  const r = richnessOf(HTML);
  if (!r.ok) console.log('\n  missing richness marks: ' + r.missing.join(', ') + '\n');
  assert.equal(r.ok, true, 'the deep-work worked example must be Fuel-grade rich');
...
test('deep-work example: it reports exactly one valid count stream', () => {
  ...
  const v = validateReport({ key: 'deep work', label: 'Deep work sessions', value: 3, date: '2026-07-02', kind: 'count', goalDirection: 'up' });
...
import { mountReady as mountDeep } from './behaviorHarness.js';

test('deep-work example behaves: renders and logging a session increments the count', async () => {
  const now = new Date('2026-07-03T10:00:00');
  const h = await mountDeep(HTML, { now });
  assert.equal(h.errors.length, 0, `the deep-work reference threw at mount: ${h.errors.join(' | ')}`);
  assert.equal(h.value(), 0, 'a fresh tile shows zero sessions');
  assert.ok(h.section().length > 0, 'the section rendered');
  await h.click('#primary');
  assert.equal(h.value(), 1, 'logging a session increments the count to 1');
  await h.click('#primary');
  assert.equal(h.value(), 2, 'a second session accumulates to 2');
  assert.equal(h.errors.length, 0, 'no throw across the interactions');
  h.close();
});

test('deep-work example behaves: reports through the host bridge and survives reopen', async () => {
  const now = new Date('2026-07-03T10:00:00');
  const h = await mountDeep(HTML, { now });
  await h.click('#primary');
  ...
  assert.equal(h2.value(), 1, 'reopening keeps the count (host persistence)');
  h.close();
  h2.close();
});
